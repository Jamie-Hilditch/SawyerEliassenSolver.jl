...
const PROFILING_FILEPATH = Base.Filesystem.joinpath(
    @__DIR__, "barotropic_niws.profile.pb.gz"
)
...
function run_simulation(Δt::Float64, OUTPUT_CADENCE::Int)
    ...
    output_writer = Tools.OutputWriter(
        OUTPUT_FILENAME, problem, state, :u, :w, :v; group=group_name(Δt), overwrite=true
    )
    output_writer()
