...
@inline trial_function(x) = sin(π * x) + cos(2 * π * x)
@inline derivative_of_trial_function(x) = π * cos(π * x) - 2 * π * sin(2 * π * x)
@inline second_derivative_of_trial_function(x) = -π^2 * sin(π * x) - 4 * π^2 * cos(2 * π * x)
@inline integral_of_trial_function(x) = -1 / π * cos(π * x) + 1 / (2 * π) * sin(2 * π * x)
@inline second_integral_of_trial_function(x) = -1 / π^2 * sin(π * x) - 1 / (4 * π^2) * cos(2 * π * x)
...
function test_physical_x_derivatives(FT)
    ...
    for x_variable_type in (XZVariable, XSVariable, XCVariable)
        ...
        @. x_variable = trial_function(x_array)
